...
namespace="io.github.vmkspv.darkstore"

print_step "Extracting translatable strings..."
xgettext --from-code=UTF-8 \
         --output=translate/darkstore.pot \
         --package-name=Darkstore \
         --copyright-holder="Vladimir Kosolapov" \
         --msgid-bugs-address="https://github.com/vmkspv/darkstore/issues" \
         --add-location=file \
         --keyword=i18n:1 \
         --keyword=i18nc:1c,2 \
         --keyword=i18np:1,2 \
...
sed -i 's/^"POT-Creation-Date:.*/"POT-Creation-Date: '"$(date +'%Y-%m-%d %H:%M%z')"'\\n"/' translate/darkstore.pot

print_success "darkstore.pot updated successfully."
...
print_info "Total translatable strings: $(grep -c msgid translate/darkstore.pot)"
